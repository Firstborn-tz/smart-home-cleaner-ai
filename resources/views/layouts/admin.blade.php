<!DOCTYPE html>
<html lang="en" class="h-full scroll-smooth" x-data="adminShell()" x-init="init()" :class="{ 'dark': darkMode }">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no, viewport-fit=cover">
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <title>@yield('title', 'Admin Dashboard') — SmartClean AI</title>
    ...
    <style>
        ...
        * { font-family: 'Inter', sans-serif; -webkit-tap-highlight-color: transparent; }
        ...
        /* Main Content */
        .main-content { margin-left: 280px; transition: margin-left 0.3s cubic-bezier(0.4, 0, 0.2, 1); }
        .main-content.expanded { margin-left: 80px; }

        /* Hover Expand */
        @media (min-width: 1024px) {
            .sidebar-wrapper:hover .sidebar.collapsed { width: 280px; }
            ...
            .sidebar-wrapper:hover .sidebar.collapsed .sidebar-badge { display: block; }
        ...
        @media (max-width: 1023px) {
            .sidebar-wrapper { position: fixed; left: -300px; transition: left 0.3s ease; width: 280px; z-index: 50; }
        ...
        <aside class="sidebar flex flex-col h-full overflow-y-auto" :class="{ 'collapsed': !sidebarOpen && !isMobile }">
            ...
            <div class="p-5 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between flex-shrink-0">
                <a href="/admin/dashboard" class="flex items-center gap-3">
                    ...
                    <div class="nav-text">
                        ...
                        <p class="text-[10px] text-muted font-medium uppercase tracking-wider">Admin Panel</p>
                    </div>
                </a>
            ...
            <div class="p-4 border-b border-gray-100 dark:border-gray-800 flex-shrink-0">
                <div class="flex items-center gap-3">
                    <img src="{{ Auth::user()->avatar_url ?? 'https://ui-avatars.com/api/?name=' . urlencode(Auth::user()->full_name ?? 'Admin') . '&background=3b82f6&color=fff&size=40&bold=true' }}" 
                         class="w-10 h-10 rounded-xl ring-2 ring-blue-100 dark:ring-blue-500/20 flex-shrink-0">
                    <div class="user-info-text min-w-0">
                        <p class="font-bold text-sm text-heading truncate">{{ Auth::user()->full_name ?? 'Administrator' }}</p>
                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-100 dark:bg-blue-500/10 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-500/20">
                            <span class="w-1.5 h-1.5 bg-blue-500 rounded-full"></span> Admin
                        </span>
                    </div>
                </div>
            </div>
            ...
            <nav class="flex-1 p-3 space-y-0.5 overflow-y-auto scrollbar-hide">
                @php
                    ...
                    $navGroups = [
                        ...
                        'Management' => [
                            ['route' => 'admin.cities.index', 'icon' => 'fa-city', 'label' => 'Cities'],
                            ['route' => 'admin.regions', 'icon' => 'fa-globe-africa', 'label' => 'Regions'],
                ...
                @php $isActive = $currentRouteName === ($item['route'] ?? '') || request()->routeIs(($item['route'] ?? '') . '*'); @endphp
                <a href="{{ isset($item['route']) && Route::has($item['route']) ? route($item['route']) : '#' }}" 
                   class="nav-link flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 group
                    ...
                    <span class="sidebar-badge bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full">{{ $item['badge'] }}</span>
                    @endif
                </a>
                @endforeach
                @endforeach
                ...
                <div class="flex items-center gap-2 sm:gap-3">
                    ...
                    <div class="hidden sm:flex relative">
                        <input type="text" placeholder="Search..." 
                               class="w-48 lg:w-64 pl-10 pr-4 py-2.5 rounded-xl border-2 border-gray-200 dark:border-gray-600 dark:bg-gray-800 text-body text-sm font-medium focus:border-blue-500 focus:ring-2 focus:ring-blue-200 dark:focus:ring-blue-800 outline-none transition-all duration-300 placeholder:text-gray-400">
                        <i class="fas fa-search absolute left-3.5 top-1/2 -translate-y-1/2 text-muted text-sm"></i>
                    </div>
                    ...
                    {{-- User Avatar --}}
                    <img src="{{ Auth::user()->avatar_url ?? 'https://ui-avatars.com/api/?name=' . urlencode(Auth::user()->full_name ?? 'A') . '&background=3b82f6&color=fff&size=36&bold=true' }}" 
